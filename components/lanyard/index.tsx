...
function Band({ maxSpeed = 50, minSpeed = 0, isMobile = false }: BandProps) {
  ...
  useFrame((state, delta) => {
    if (dragged && typeof dragged !== 'boolean') {
      ...
      // Prevent unstable dragging when camera distance is too close to zero.
      vec.add(dir.multiplyScalar(Math.max(state.camera.position.length(), 8)));
  ...
  return (
    <>
      <group position={[0, 4, 0]}>
        ...
        >
          <CuboidCollider args={[CARD_COLLIDER_HALF_WIDTH, CARD_COLLIDER_HALF_HEIGHT, 0.01]} />
          <group
            scale={CARD_SCALE}
            position={[0, -1.2, -0.05]}
